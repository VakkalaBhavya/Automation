...
public class StepDefinition{
	...
	public void closeBrowser() throws Exception {		
		try
		{
			Messages.getMessages().add(Thread.currentThread().getId() +"Close the Browser");
			
			con.closeDriver();
			Messages.getMessages().add(Thread.currentThread().getId() +" PASS : Close the Browser");
		}
		catch(Exception e)
		{
			Messages.getMessages().add(Thread.currentThread().getId() +" Failed to Close the Browser");
			e.printStackTrace();
		}
	}
